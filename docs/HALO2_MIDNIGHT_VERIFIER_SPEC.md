...
- `Halo2VerifyingKey`: a data-only contract whose runtime bytecode is the VK
  payload.
...
The VK contract constructor writes the payload into memory and returns exactly
that payload as runtime bytecode. The verifier:

1. Stores the expected VK runtime length.
...
is exactly the runtime bytecode of `Halo2VerifyingKey`.
...
0x1a:
    native trash callback, currently experimental
