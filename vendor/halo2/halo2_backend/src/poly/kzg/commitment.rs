...
where
    ...
    E::G1: CurveExt<AffineExt = E::G1Affine>,
{
    // ----- Patched accessors (halo2-solidity-verifier-exp) -----------------
    // Upstream keeps `g`, `g2`, and `s_g2` `pub(crate)` so downstream callers
    // (the Solidity codegen) cannot read the SRS commitments. We add public
    // read-only views; nothing about ownership or mutation changes.

    /// Public accessor (patched in): the SRS G1 generators.
    pub fn g(&self) -> &[E::G1Affine] {
        &self.g
    }

    /// Public accessor (patched in): the SRS G2 generator.
    pub fn g2(&self) -> E::G2Affine {
        self.g2
    }

    /// Public accessor (patched in): the SRS toxic-waste-shifted G2 generator.
