...
///
/// Patched in halo2-solidity-verifier-exp: upstream marks this struct as
/// `pub(crate)`, which means downstream code (Solidity codegen) cannot read
/// the permutation commitments. We promote the struct + add a public
/// `commitments()` accessor below.
#[derive(Clone, Debug)]
...
impl<C: CurveAffine> VerifyingKey<C> {
    /// Public accessor (patched in): returns the permutation argument's
    /// commitment vector. Each entry corresponds to one permutation column.
