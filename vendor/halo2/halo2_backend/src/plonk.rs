...
impl<C: CurveAffine> VerifyingKey<C> {
    ...
    /// Returns the permutation argument's verifying key (commitments).
    ///
    /// Patched in halo2-solidity-verifier-exp: upstream keeps this field
    /// `pub(crate)` so downstream callers can't read the per-column
    /// permutation commitments. The Solidity codegen needs them to lay
    /// out the permutation section of `Halo2VerifyingKey.sol`.
    ...
    ///
    /// Patched in halo2-solidity-verifier-exp: upstream is `pub(crate)`.
    /// The Solidity codegen needs to walk the constraint system to compute
    /// `ConstraintSystemMeta` (column counts, queries, permutation cols, ...).
