...
contract Halo2Verifier {
    ...
    function verifyProof(
        ...
        assembly ("memory-safe") {
            ...
            //
            // TranscriptRenderPlan emits the proof G1 reads from the typed
            // TranscriptPlan and ProofCalldataLayout: each proof G1 validates and absorbs that 128-byte form, then copies it into its planned
            // MPTR destination. The generated plan also squeezes
            // trash_challenge unconditionally before any guarded trashcan
            // commitment reads.
            //
            // Under truncated-challenges, x3 is masked immediately after
            // squeeze; x1 and x4 remain full squeezed Fr words, while PCS
            // later stores truncate(x1^i) and truncate(x4^i) for powers.
            // ===============================================================
            {%- for line in codegen_layout.transcript_render.lines %}
            {{ line }}
            {%- endfor %}
